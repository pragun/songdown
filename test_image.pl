...
my @chord_input = ("2o43x2","F");
my $num_strings = 6;
my $min_num_frets_to_display = 3;
my @tuning = split("","EADGBE");
...
my ($fingering,$chord_name) = @chord_input;
my @fingering = split("",$fingering);
if (@fingering != $num_strings){
    print "Chord input does not match number of strings:@chord_input\n";
